...
fn criterion_benchmark(c: &mut Criterion) {
    ...
    c.bench_function("evaluate long expression crunch_eval no flatten", |b| {
        let expr = Expr::<f32, 0>::compile("6.5*7.8^2.3 + (3.5^3+7/2)^3 -(5*4/(2-3))*4 + 6.5*7.8^2.3 + (3.5^3+7/2)^3 -(5*4/(2-3))*4 + 6.5*7.8^2.3 + (3.5^3+7/2)^3 -(5*4/(2-3))*4 + 6.5*7.8^2.3 + (3.5^3+7/2)^3 -(5*4/(2-3))*4", Default::default());
    ...
    c.bench_function(
        ...
        |b| {
            ...
            let expr = Expr::compile("double(x + 1)", env).unwrap();
            let vars = &[25.0];
